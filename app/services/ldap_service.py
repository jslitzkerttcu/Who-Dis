...
class LDAPService:
    def __init__(self):
        ...
        self.connect_timeout = int(os.getenv('LDAP_CONNECT_TIMEOUT', '5'))  # Connection timeout in seconds
        self.operation_timeout = int(os.getenv('LDAP_OPERATION_TIMEOUT', '10'))  # Operation timeout in seconds
    ...
    def search_user(self, search_term: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Timeouts - Connect: {self.connect_timeout}s, Operation: {self.operation_timeout}s")
            ...
            with Connection(server, user=self.bind_dn, password=self.bind_password, auto_bind=True,
                          receive_timeout=self.operation_timeout) as conn:
                    ...
                    if len(conn.entries) > 1:
                        ...
                        for entry in conn.entries[:10]:  # Limit to 10 results
                            try:
                                ...
                                logger.error(f"Error processing entry {entry.entry_dn}: {str(e)}")
            ...
            raise TimeoutError(f"LDAP operation timed out after {self.operation_timeout} seconds. Please try a more specific search term.")
    ...
    def _process_ldap_entry(self, entry) -> Dict[str, Any]:
            ...
            try:
                ...
                if isinstance(pwd_last_set_raw, datetime):
                    ...
                    if pwd_last_set_value > 0:
                        ...
                        unix_timestamp = (pwd_last_set_value - 116444736000000000) / 10000000
        ...
        if pwd_expiry_attr and hasattr(pwd_expiry_attr, 'value') and pwd_expiry_attr.value:
            try:
                ...
                if pwd_expiry_value > 0 and pwd_expiry_value != 9223372036854775807:  # Never expires value
        ...
        if result.get('pwdLastSet') or result.get('pwdExpires'):
            logger.info(f"Returning LDAP data with pwdLastSet: {result.get('pwdLastSet')}, pwdExpires: {result.get('pwdExpires')}")
