...
class DataWarehouseCache(CacheableModel):
    ...
    @classmethod
    def cache_user_data(cls, upn: str, user_data: Dict[str, Any]) -> 'DataWarehouseCache':
        """
        ...
        record.keystone_expected_role = user_data.get('Keystone_Expected_Role_For_Job_Title')
    ...
    def get_keystone_info(self) -> Dict[str, Any]:
        ...
        if self.keystone_expected_role and self.live_role:
            if self.live_role != self.keystone_expected_role:
                role_mismatch = f"Expected: {self.keystone_expected_role}, Actual: {self.live_role}"
        ...
        return {
            ...
            'last_login': self.ks_last_login_time.isoformat() if self.ks_last_login_time else None,
