...
def is_sensitive_field(config_key):
    ...
    try:
        ...
        is_sensitive = any(suffix in key.lower() for suffix in ["secret", "password"])
        ...
        if not value:
            ...
            with db.engine.begin() as conn:
                ...
                for test_key in keys_to_try:
                    result = conn.execute(
                        text(
                            "SELECT encrypted_value FROM configuration WHERE category = :category AND setting_key = :key"
                        ),
...
def render_app_config():
    ...
    timezone = config_get("app.timezone", "US/Central")
    ...
    <form hx-post="/admin/api/configuration?section=app" 
        ...
        <!-- System Settings -->
        <h6 class="text-lg font-semibold text-gray-900 border-b border-gray-200 pb-2 mb-4 mt-6">System Configuration</h6>
        <div class="grid md:grid-cols-2 gap-4 mb-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Server Timezone</label>
                <select name="app_timezone" 
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-ttcu-green">
                    <option value="US/Eastern" {"selected" if timezone == "US/Eastern" else ""}>Eastern Time (ET)</option>
                    <option value="US/Central" {"selected" if timezone == "US/Central" else ""}>Central Time (CT)</option>
                    <option value="US/Mountain" {"selected" if timezone == "US/Mountain" else ""}>Mountain Time (MT)</option>
                    <option value="US/Pacific" {"selected" if timezone == "US/Pacific" else ""}>Pacific Time (PT)</option>
                    <option value="US/Alaska" {"selected" if timezone == "US/Alaska" else ""}>Alaska Time (AKT)</option>
                    <option value="US/Hawaii" {"selected" if timezone == "US/Hawaii" else ""}>Hawaii Time (HST)</option>
                    <option value="US/Arizona" {"selected" if timezone == "US/Arizona" else ""}>Arizona Time (MST no DST)</option>
                    <option value="US/Indiana-Starke" {"selected" if timezone == "US/Indiana-Starke" else ""}>Indiana-Starke Time</option>
                    <option value="US/Michigan" {"selected" if timezone == "US/Michigan" else ""}>Michigan Time</option>
                    <option value="UTC" {"selected" if timezone == "UTC" else ""}>UTC (Coordinated Universal Time)</option>
                </select>
                <p class="text-xs text-gray-500 mt-1">Timezone for displaying timestamps throughout the application</p>
            </div>
            <div>
