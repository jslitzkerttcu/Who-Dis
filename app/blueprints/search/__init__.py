...
SEARCH_OVERALL_TIMEOUT = int(os.getenv('SEARCH_OVERALL_TIMEOUT', '20'))  # Overall timeout in seconds
...
def merge_ldap_graph_data(ldap_data, graph_data):
    ...
    if 'thumbnailPhoto' in merged:
        logger.info(f"Removing LDAP thumbnail photo from merged data")
    ...
    if graph_data:
        ...
        if graph_data.get('photoUrl'):
            logger.info(f"Setting thumbnailPhoto from Graph photoUrl")
            merged['thumbnailPhoto'] = graph_data['photoUrl']
        else:
            logger.info(f"No photoUrl in Graph data")
        ...
        if graph_data.get('lastPasswordChangeDateTime'):
            merged['lastPasswordChangeDateTime'] = graph_data['lastPasswordChangeDateTime']
        ...
        if graph_data.get('refreshTokensValidFromDateTime'):
            merged['refreshTokensValidFromDateTime'] = graph_data['refreshTokensValidFromDateTime']
        if graph_data.get('signInSessionsValidFromDateTime'):
            merged['signInSessionsValidFromDateTime'] = graph_data['signInSessionsValidFromDateTime']
        if graph_data.get('onPremisesLastSyncDateTime'):
            merged['onPremisesLastSyncDateTime'] = graph_data['onPremisesLastSyncDateTime']
...
def search_user():
    ...
    genesys_user_id = data.get('genesys_user_id')  # For when user selects from multiple results
    ldap_user_dn = data.get('ldap_user_dn')  # For when user selects from multiple LDAP results
    graph_user_id = data.get('graph_user_id')  # For when user selects from multiple Graph results
    
    if not search_term:
        return jsonify({'error': 'Search term is required'}), 400
    
    logger.info(f"Searching for user: {search_term}, specific genesys_id: {genesys_user_id}")
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        if genesys_user_id:
            genesys_future = executor.submit(genesys_service.get_user_by_id, genesys_user_id)
        else:
        ...
        try:
            ...
            if ldap_user_dn and ldap_data:
                ...
                if isinstance(ldap_data, dict) and ldap_data.get('multiple_results'):
                    ldap_multiple = True
                    ldap_result = ldap_data
                    logger.info(f"LDAP search for '{search_term}' - Multiple results: {len(ldap_data.get('results', []))}")
                else:
                    # Single result
                    ldap_result = ldap_data
            ...
            logger.error(f"LDAP search timed out after {SEARCH_OVERALL_TIMEOUT} seconds")
        ...
        try:
            ...
            if genesys_user_id and genesys_data:
                ...
                if isinstance(genesys_data, dict):
                    if genesys_data.get('error') == 'too_many_results':
                        ...
                        logger.info(f"Genesys search for '{search_term}' - Too many results: {genesys_data.get('total')}")
                    elif genesys_data.get('multiple_results'):
                        genesys_multiple = True
                        genesys_result = genesys_data
                        logger.info(f"Genesys search for '{search_term}' - Multiple results: {len(genesys_data.get('results', []))}")
                    else:
                        # Single result already processed
                        genesys_result = genesys_data
                        logger.info(f"Genesys search for '{search_term}' - Found single user")
                else:
                    genesys_result = genesys_data
            else:
            ...
            logger.error(f"Genesys search timed out after {SEARCH_OVERALL_TIMEOUT} seconds")
        ...
        try:
            ...
            if graph_user_id and graph_data:
                ...
                if isinstance(graph_data, dict) and graph_data.get('multiple_results'):
                    graph_multiple = True
                    graph_result = graph_data
                    logger.info(f"Graph API search for '{search_term}' - Multiple results: {len(graph_data.get('results', []))}")
                else:
                    # Single result
                    graph_result = graph_data
                    logger.info(f"Graph API search for '{search_term}' - Found single user")
            else:
                logger.info(f"Graph API search for '{search_term}' - No results")
                
        except FutureTimeoutError:
            logger.error(f"Graph API search timed out after {SEARCH_OVERALL_TIMEOUT} seconds")
    ...
    if ldap_error and genesys_error and graph_error and "timed out" in str(ldap_error) and "timed out" in str(genesys_error) and "timed out" in str(graph_error):
        return jsonify({
    ...
    # Handle multiple results - need to match and merge
        ...
        logger.info(f"Handling multiple results - LDAP multiple: {ldap_multiple}, Graph multiple: {graph_multiple}")
        # Smart matching when we have single LDAP + multiple Graph results
        if ldap_result and not ldap_multiple and graph_multiple and graph_result and graph_result.get('results'):
            logger.info(f"Smart matching: Single LDAP + Multiple Graph results")
            ...
            if ldap_email:
                ...
                if matched_graph:
                    ...
                    if graph_user_id:
                        ...
                        if full_graph_user:
                            logger.info(f"Full Graph user data fields: {list(full_graph_user.keys())}")
                            if 'photoUrl' in full_graph_user:
                                logger.info(f"Graph photo URL present: Yes")
                            else:
                                logger.info(f"Graph photo URL present: No")
                            if 'lastPasswordChangeDateTime' in full_graph_user:
                                logger.info(f"Password change date: {full_graph_user['lastPasswordChangeDateTime']}")
                            if 'createdDateTime' in full_graph_user:
                                logger.info(f"Created date: {full_graph_user['createdDateTime']}")
                            azure_ad_result = merge_ldap_graph_data(ldap_result, full_graph_user)
                            logger.info(f"Smart match: Found Graph user {matched_graph.get('displayName')} matching LDAP email {ldap_email}")
                            logger.info(f"Merged result has Graph data: {azure_ad_result.get('hasGraphData', False)}")
                            logger.info(f"Merged result has thumbnailPhoto: {'thumbnailPhoto' in azure_ad_result}")
                        else:
                            logger.warning(f"Failed to fetch full Graph user details for ID: {graph_user_id}")
                            # Fallback to merging with basic data
                            azure_ad_result = merge_ldap_graph_data(ldap_result, matched_graph)
                    else:
                        logger.warning(f"No Graph user ID found in matched result")
                        # Fallback to merging with basic data
                        azure_ad_result = merge_ldap_graph_data(ldap_result, matched_graph)
                else:
        ...
        # Smart matching when we have multiple LDAP + single Graph result
        elif ldap_multiple and ldap_result and ldap_result.get('results') and graph_result and not graph_multiple:
            ...
            if graph_email:
                ...
                if matched_ldap:
                    ...
                    logger.info(f"Smart match: Found LDAP user {matched_ldap.get('displayName')} matching Graph email {graph_email}")
                else:
                    # No match found, show multiple results
                    azure_ad_multiple = True
        ...
        # Both have multiple results
            ...
            azure_ad_result = {
                ...
                'graph_results': graph_result.get('results', []) if graph_result else [],
    ...
    if ldap_error and graph_error:
        azure_ad_error = "Both Active Directory and Microsoft Graph searches encountered errors"
    elif ldap_error:
    ...
    if (azure_ad_result and not azure_ad_multiple and 
        genesys_multiple and genesys_result and genesys_result.get('results')):
        
        # Get the email from the single AD result
        ad_email = azure_ad_result.get('mail')
        
        if ad_email:
            logger.info(f"Smart matching: Single AD result with email {ad_email}, checking {len(genesys_result['results'])} Genesys results")
            ...
            for g_user in genesys_result['results']:
                if g_user.get('email', '').lower() == ad_email.lower():
                    matched_user = g_user
                    logger.info(f"Smart match found: Genesys user {g_user.get('name')} matches AD email {ad_email}")
                    break
            
            # If we found a match, get the full user details
            if matched_user and matched_user.get('id'):
                try:
                    full_genesys_user = genesys_service.get_user_by_id(matched_user['id'])
                    if full_genesys_user:
                        genesys_result = full_genesys_user
                        genesys_multiple = False
                        logger.info(f"Smart match successful: Retrieved full Genesys user details for {matched_user.get('name')}")
                except Exception as e:
