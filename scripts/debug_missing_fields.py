...
def main():
    ...
    print("-" * 80)
    print(f"{'Category':<10} {'Key':<25} {'Plain':<10} {'Encrypted':<10} {'Sensitive':<10}")
    print("-" * 80)
    ...
    for category, key, plain, encrypted, sensitive in cursor.fetchall():
        ...
        print(f"{category:<10} {key:<25} {has_plain:<10} {has_encrypted:<10} {sensitive!s:<10}")
        ...
        if key in ['bind_dn', 'client_id']:
            ...
            if encrypted:
                encrypted_preview = str(encrypted)[:50] + "..." if len(str(encrypted)) > 50 else str(encrypted)
