...
def test_config_decryption():
    ...
    if "WHODIS_ENCRYPTION_KEY" in os.environ:
        print(
            f"WHODIS_ENCRYPTION_KEY length: {len(os.environ['WHODIS_ENCRYPTION_KEY'])}"
        )
    ...
    with app.app_context():
        ...
        print(
            f"  client_id: {'✓' if client_id else '✗'} {repr(client_id) if client_id else 'None'}"
        )
        print(
            f"  client_secret: {'✓' if client_secret else '✗'} {'*' * 8 if client_secret else 'None'}"
        )
        print(
            f"  tenant_id: {'✓' if tenant_id else '✗'} {repr(tenant_id) if tenant_id else 'None'}"
        )
        ...
        for key in ["graph.client_id", "graph.client_secret", "graph.tenant_id"]:
            ...
            if value and value.startswith("gAAAAA"):
                ...
                print("  This means decryption failed.")

        # Test LDAP configuration
        print("\n\nTesting LDAP configuration:")
        ldap_password = config_get("ldap.bind_password")
        print(
            f"  bind_password: {'✓' if ldap_password else '✗'} {'*' * 8 if ldap_password else 'None'}"
        )
